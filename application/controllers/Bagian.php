...
class Bagian extends Auth_Controller {
	...
	{	
		...
		$data['surat_masuk'] = $this->bagian->tampil_suratmasuk()->result();

		
		$this->load->view('bagian/v_SM', $data);
	}

	public function tambahSM()
	{
		$data['title'] = 'Tambah Data Surat Masuk';
		$disposisi = $this->bagian->disposisi()->result();
		$data['disposisi'] = $disposisi;
		$data['nomorurut_suratmasuk'] = $this->_ambil_nomorSM();

		$this->load->view('bagian/v_SMtambah', $data);
	...
	{		
		if ($this->input->method() === 'post') {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				return $this->load->view('bagian/v_SMtambah', $data);
			}
	...
	{
		$data['surat_masuk'] = $this->bagian->SM_by_id($id);
		$data['title'] = 'Edit data Surat Masuk';

		// $disposisi = $this->bagian->disposisi()->result();
		$data['disposisi'] = $this->bagian->disposisi()->result();

		$this->load->view('bagian/v_SMedit', $data);
	}
	...
	public function edit_prosesSM() 
	{
		if ($this->input->method() === 'post') {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				return $this->load->view('bagian/v_SMedit', $data);
			}
			date_default_timezone_set('Asia/Jakarta'); 
					$thnNow = date("Y");
			$nomorurut_suratmasuk = $this->input->post('nomorurut_suratmasuk');
			...
			if(!empty($_FILES['file_suratmasuk']['name'])){		
				...
				//delete file
				$SM = $this->bagian->SM_by_id($this->input->post('id_suratmasuk'));
			...
			$result = $this->bagian->SM_edit($SM_edit, $id_suratmasuk);
	...
	{
		$data1 = $this->bagian->nomorSM()->result();
		$jumlah = $this->bagian->nomorSM()->num_rows();
	...
	public function tambahSK()
	{
		...
		$this->load->view('bagian/v_SKtambah', $data);
	...
	{		
		if ($this->input->method() === 'post') {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				return $this->load->view('bagian/v_SKtambah', $data);
			}
			...
			move_uploaded_file($tmp_file, $path);
					...
					$result = $this->bagian->SK_tambah($SK_baru);
					if ($result > 0) {
						$this->session->set_flashdata('message', '<div class="alert alert-success alert-dismissible show" role="alert">
							Data Surat Keluar berhasil ditambahkan.
							<a href="#" class="close text-white" data-dismiss="alert" aria-label="close">&times;</a>
						</button></div>');
						redirect("bagian/surat_keluar");
					} else {
	...
	{
		$data1 = $this->bagian->nomorSK()->result();
		$jumlah = $this->bagian->nomorSK()->num_rows();
	...
	{
		$data['surat_keluar'] = $this->bagian->SK_by_id($id);
		$data['title'] = 'Edit data Surat Keluar';

		// $disposisi = $this->bagian->disposisi()->result();
		$data['disposisi'] = $this->bagian->disposisi()->result();

		$this->load->view('bagian/v_SKedit', $data);
	}
	...
	public function edit_prosesSK() {
		if ($this->input->method() === 'post') {
			...
			if(!empty($_FILES['file_suratkeluar']['name'])){		
				...
				//delete file
				$SK = $this->bagian->SK_by_id($this->input->post('id_suratkeluar'));
			...
			$id_surat = $this->input->post('id_suratkeluar');
					...
					$result = $this->bagian->SK_edit($SK_edit, $id_suratkeluar);
					if ($result > 0) {
						$this->session->set_flashdata('message', '<div class="alert alert-success alert-dismissible show" role="alert">
	...
	public function hapusSK($id)
	{
		...
		$hapusSK = $this->bagian->SK_hapus($data['id_suratkeluar']);
...
/*
	...
	public function bagian ()
	{
		...
		$data['data_bagian'] = $this->bagian->tampil_bagian()->result();
	...
	public function bagian_tambah() 
	{
		...
		$this->load->view('bagian/v_bagianTambah', $data);
	}
	...
	public function bagian_proses_tambah()
	{
		if ($this->input->method() === 'post') {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$data['error']= ' ';
				return $this->load->view('bagian/v_bagianTambah', $data);
			}
			...
			$data_bagian = array('nama_bagian' => $this->input->post('nama_bagian'),
								'username_bagian_bagian' => $this->input->post('username_bagian_bagian'),
			...
			$result = $this->bagian->bagian_tambah($data_bagian);
				if ($result > 0) {
					$this->session->set_flashdata('message', '<div class="alert alert-success alert-dismissible show" role="alert">
	...
	{
		$data['bagian'] = $this->bagian->bagian_by_id($id);
		$data['title'] = 'Edit data Bagian';


		$this->load->view('bagian/v_bagianEdit', $data);
	}
	...
	public function bagian_proses_edit()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$data['error']= ' ';
			return $this->load->view('bagian/editBagian/'.$this->input->post('id_bagian'), $data);
		}
		...
		if(!empty($_FILES['gambar']['name'])){
			...
			//delete file
			$bagian = $this->bagian->bagian_by_id($this->input->post('nim'));
		...
		$result = $this->bagian->bagian_edit($data_edit, $id_bagian);
		if ($result > 0) {
			$this->session->set_flashdata('message', '<div class="alert alert-success alert-dismissible show" role="alert">
	...
	{
		$username = $this->input->post('username_bagian_bagian');
	...
	{
		$bagianlist = $this->bagian->bagian_by_id($id);
		...
		$hapusbagian = $this->bagian->bagian_hapus($data['id_bagian']);
